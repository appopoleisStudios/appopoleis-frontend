...
function Header() {
    ...
    return (
        <>
        <div className='fixed bg-white w-full flex justify-between items-center py-3 px-6 lg:px-10 z-10 border-b border-zinc-400 -top-1'>
            <div className='flex items-center' style={{width:"12%"}}>
                <img src={logo} alt="logo" className='h-25' style={{width:"100%"}} />
            </div>
            <div className='flex items-center gap-4 md:gap-8 font-semibold text-sm'>
                ...
                <Link to={'/'} className='bg-[#3F4CCC] px-6 py-3 font-medium text-white rounded-sm'>Contact Us</Link>
                <div>
                    <AlignLeft strokeWidth={3} color='black' className='block lg:hidden cursor-pointer' onClick={toggleMenu} />
        ...
        <div className={`fixed top-0 right-0 z-30 w-80 h-full bg-white shadow-lg p-6 transform transition-transform duration-300 ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
                ...
                <nav className="flex flex-col gap-6 text-lg font-semibold">
                    ...
                    <Link href='/' onClick={closeMenu}>Process</Link>
                    <Link href='/' onClick={closeMenu}>Insights</Link>
                    <Link href='/' onClick={closeMenu}>FATbit</Link>
                    <Link href='/' onClick={closeMenu}>Hire Our Experts</Link>
                </nav>
